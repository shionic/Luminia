...
TaskService.getResultById(taskAssignId).then(t => {
    taskResult.value = t;
    ...
    <main>
        ...
        <div class="tv-container">
            ...
            <div class="tv-container-element">
                ...
                <FileView v-if="!uploadMode && taskResult?.isOk()" v-for="a in taskResult?.data?.attachments" v-bind:key="a.id" :attachment="a"></FileView>
                <FileView v-for="a in uploadFiles" v-bind:key="a.id" :attachment="a"></FileView>
        <div class="upload-container card">
            <div class="hcenter">
                    <input ref="uploadInput" type="file" id="avatar" name="avatar" accept="*" /> <l-button type="primary" @click="uploadFile">Загрузить файл</l-button> <l-button v-if="uploadFiles.length > 0" type="primary" @click="uploadResult">Загрузить решение</l-button>
            </div>
        </div>
            </div>
        </div>
